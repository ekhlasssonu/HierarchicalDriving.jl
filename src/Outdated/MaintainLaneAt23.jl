...
=#
function sortNeighborhood_M23(neighborhood::Array{Array{CarLocalISL0,1},1})
  ...
  for ln in 1:numLanes
    for carIS in neighborhood[ln]
      ...
      carLane = getLaneNo_M23(carPhySt)
...
function getOtherCarsAction_M23(globalISL1::GlobalStateL1, rng::AbstractRNG)
  #=
  ...
  egoLane = getLaneNo_M23(egoState)
...
function updateNeighborState_M23(globalISL1::GlobalStateL1, rng::AbstractRNG)
  #Maneuver specific
  laneCenters = [-3*LANE_WIDTH/2, -LANE_WIDTH/2, LANE_WIDTH/2, 3*LANE_WIDTH/2]

  #Maneuver independent
  egoLane = getLaneNo_M23(globalISL1.ego)
  numLanes = length(globalISL1.neighborhood)
  ...
  actions = getOtherCarsAction_M23(globalISL1, rng) #This gives all cars' (sampled) action in the order they are on neighborhood
  ...
  return sortNeighborhood_M23(updatedNeighborhood)
...
=#
type MaintainAt23POMDP <:POMDPs.POMDP{GlobalStateL1, Int64, EgoObservation}
...
MaintainAt23POMDP() = MaintainAt23POMDP(0.9, -0.5, -50.0, 0.0, -3.0, -2.0, -0.2, 0.0, AVG_HWY_VELOCITY-2.2352)
discount(p::MaintainAt23POMDP) = p.discount_factor
isterminal(::MaintainAt23POMDP, act::Int64) = act == length(EgoActionSpace().actions)
#Needs to be updated. No need for absent
function isterminal(p::MaintainAt23POMDP, st::GlobalStateL1)
  st.terminal ? true : false
end
#From actions
n_actions(p::MaintainAt23POMDP) = length(actions(p))
actions(::MaintainAt23POMDP) = EgoActionSpace()

#=
Generate state

=#
type MaintainLaneAt23NormalStateDist
...
Base.eltype(::MaintainLaneAt23NormalStateDist) = GlobalStateL1

function initial_state_distribution(p::MaintainAt23POMDP)
  ...
  return(MaintainLaneAt23NormalStateDist(egoDist, lLDist, cLDist, rLDist))
end

function rand(rng::AbstractRNG, d::MaintainLaneAt23NormalStateDist, frameList::Array{CarFrameL0,1}=getFrameList())
...
function generate_o(p::MaintainAt23POMDP, s::Union{GlobalStateL1,Void}, a::Union{Int64,Void}, sp::GlobalStateL1, rng::AbstractRNG)
...
#Generate next state
function generate_s(p::MaintainAt23POMDP, s::GlobalStateL1, a::Int64, rng::AbstractRNG)
  ...
  neighborhood = updateNeighborState_M23(s, rng)
...
#Generate reward
function reward(p::MaintainAt23POMDP, s::GlobalStateL1, a::Int64, rng::AbstractRNG)
  actionSet = EgoActionSpace()
  act = actionSet.actions[a]
  if (s.terminal )
    return 0.0
  ...
  nbrActions = getOtherCarsAction_M23(s, rng) #Randomness is iffy but IDM part is constant, so no problem there
  discomfort = false
...
function generate_sor(p::MaintainAt23POMDP, s::GlobalStateL1, a::Int64, rng::AbstractRNG)
...
function initial_state(p::MaintainAt23POMDP, rng::AbstractRNG)
  isd = initial_state_distribution(p)
  return rand(rng, isd)
end

type MaintainAt23LowerBound
    rng::AbstractRNG
end

type MaintainAt23UpperBound
    rng::AbstractRNG
end

function lower_bound(lb::MaintainAt23LowerBound, p::MaintainAt23POMDP, s::GlobalStateL1)
    return p.collisionCost
end

function upper_bound(ub::MaintainAt23UpperBound, p::MaintainAt23POMDP, s::GlobalStateL1)
    return 0.0
end

function init_lower_action(p::MaintainAt23POMDP)
