...
collision(s1::CarPhysicalState, is2::CarLocalIS, safetyDist::Float64=0.0) = (abs(s1.state[1] - is2.physicalState.state[1]) < (CAR_LENGTH + is2.model.frame.carLength)/2 + safetyDist) && (abs(s1.state[2] - is2.physicalState.state[2]) < (CAR_WIDTH + is2.model.frame.carWidth)/2 + 0.125 * safetyDist)
collision(is1::CarLocalIS, is2::CarLocalIS, safetyDist::Float64=0.0) = (abs(is1.physicalState.state[1] - is2.physicalState.state[1]) < (is1.model.frame.carLength + is2.model.frame.carLength)/2 + safetyDist) && (abs(is1.physicalState.state[2] - is2.physicalState.state[2]) < (is1.model.frame.carWidth + is2.model.frame.carWidth)/2 + 0.125 * safetyDist)
...
function createFSM()
  ...
  actionProb[(nodeSet[1], 0.0)] = 0.5  #1: Keep straight with prob 0.5; Dummy node anyway
  actionProb[(nodeSet[1], 2.0)] = 0.5  #1: Move toward target with prob 0.5; Dummy node anyway
  ...
  transitionProb[(nodeSet[2], edgeLabels[2], nodeSet[4])] = 1.0
  transitionProb[(nodeSet[2], edgeLabels[3], nodeSet[2])] = 1.0
  transitionProb[(nodeSet[2], edgeLabels[4], nodeSet[2])] = 1.0

  transitionProb[(nodeSet[3], edgeLabels[1], nodeSet[2])] = 1.0 #Should receive this on reaching the center of the current lane
  ...
  transitionProb[(nodeSet[3], edgeLabels[4], nodeSet[3])] = 1.0 #Undeterminable, stay here with prob. 1.0
