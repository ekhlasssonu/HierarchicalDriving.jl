...
typealias NbCache Array{Array{SVector{2,CarPhysicalState},1},1}
...
type GlobalStateL1{C <: CarLocalIS}
  ...
  _neighbor_cache::Nullable{NbCache} # first index is car position in neighborhood +1, the first entry is for ego. USE getImmediateNeighbors FOR ACCESS
end
GlobalStateL1(ego::CarPhysicalState, neighborhood::Array{Array{CarLocalIS,1},1}) = GlobalStateL1(0, ego, neighborhood, Nullable{NbCache}())
GlobalStateL1(term, ego::CarPhysicalState, neighborhood::Array{Array{CarLocalIS,1},1}) = GlobalStateL1(term, ego, neighborhood, Nullable{NbCache}())
